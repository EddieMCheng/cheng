(function() {
  ...
  function startup() {
    initiateButton = document.getElementById('initiateButton');
    ...
    initiateButton.addEventListener('click', initiate, false);
  ...
  function initiate(){
    ...
    if(callerText.value==""){
      ...
      .then(() =>{ 
          let jsonDesc=JSON.stringify(connection.localDescription);
      ...
      remoteConnection.setRemoteDescription(new RTCSessionDescription(JSON.parse(callerText.value)));
      remoteConnection.createAnswer()
      .then(answer => remoteConnection.setLocalDescription(answer))
      .then(() => {
        let jsonDesc=JSON.stringify(remoteConnection.localDescription);
          console.log("answer: "+jsonDesc);
          answerText.value=jsonDesc;
          connection.setRemoteDescription(new RTCSessionDescription(JSON.parse(answerText.value)));
      })
    }

  }
  function connectPeers() {
    //remoteConnection.addIceCandidate(new RTCIceCandidate(JSON.parse(localCandidateText.value)));
   // remoteConnection.addIceCandidate(new RTCIceCandidate(JSON.parse(localCandidate2Text.value)));
  //  connection.addIceCandidate(new RTCIceCandidate(JSON.parse(remoteCandidateText.value)));
   // connection.addIceCandidate(new RTCIceCandidate(JSON.parse(remoteCandidate2Text.value)));
